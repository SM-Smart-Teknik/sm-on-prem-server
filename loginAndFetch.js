...
async function getCachedCookie() {
  try {
    ...
    console.log("🔸 Cached cookie expired");
    return null;
  } catch (error) {
    console.log("🔸 No cached cookie found");
    return null;
  }
}
...
async function saveCookie(cookie) {
  ...
  console.log("🔹 Saved cookie to cache");
}
...
async function loginAndGetCookies() {
  ...
  try {
    ...
    console.log("🔹 Got session cookie:", sessionCookie.value);
...
async function fetchWorkOrders(sessionId) {
  const userId = 32;
...
async function main() {
  try {
    const sessionId = await getValidSessionCookie();
    const workOrders = await fetchWorkOrders(sessionId);
